...
class AwsLambda {
   ...
   * @param userToken UserToken (_id) of the newly registered user
   * @return {Promise}
   */
  static sendWelcomeMail(/*string*/ email, /*string*/ name, /*string*/ userToken) {
    ...
    if (!userToken) return Promise.reject('userToken is empty!').catch(e => log.error(e));

    if (process.env.NODE_ENV === 'test') return Promise.resolve();
    return request(this._getOptionsForWelcomeMail(email, name, userToken));
  }

  /**
   * @private
   */
  static _getOptionsForWelcomeMail(/*string*/ email, /*string*/ name, /*string*/ userToken) {
    return this._getOptions(config.awsLambda.welcomeMailUrl, {email, name, id: userToken});
  }
